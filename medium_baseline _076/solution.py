...
class BO_algo():
    def __init__(self):
        ...
        self.gp_f = GaussianProcessRegressor(kernel=ConstantKernel(constant_value=0.5) * RBF(length_scale=1), alpha=self.sigma_f**2)
    ...
    def acquisition_function(self, x: np.ndarray):
        ...
        if mean_v + 4*std_v >= SAFETY_THRESHOLD:
            af_value = af_value - self.lamb * (mean_v + 4*std_v - SAFETY_THRESHOLD)
